  ...
  if (time_window_mc = dynamic_cast<JPetTimeWindowMC *const>(fEvent)) {
   ...
   if (auto timeWindow = dynamic_cast<const JPetTimeWindow *const>(fEvent)) {
    ...
    for (uint i = 0; i < timeWindow->getNumberOfEvents(); i++) {
      ...
      //int barrel_size = getParamBank().getSlotsSize();
  ...
  if ( error == 0){
	...
	getStatistics().fillHistogram(("hitsDis_diff_afterEngDep_"+ EventTypeNames.at(event_type)).c_str(), hitsDis_diff(hits).at(0), hitsDis_diff(hits).at(2) );
	  ...
	  if ( (thetas[0]+ thetas[1]) > 180){
	    ...
	    /*	 	 
	    if ( ellipse_cut_values( angle_sum, calculatedLOR( hits, event_type)) == true ){

	      getStatistics().fillHistogram( "HitMult_afterEllipCut", event_type);

	      getStatistics().fillHistogram(("sum_diff_angles_afterEllipCut_"+ EventTypeNames.at(event_type)).c_str(), (theta.at(0)+ theta.at(1))*TMath::RadToDeg(), (theta.at(1)- theta.at(0))*TMath::RadToDeg());

	      //getStatistics().fillHistogram(("scattTest_afterEllipCut_"+ EventTypeNames.at(event_type)).c_str(), scatterTest( event, event_type ) );

	      //getStatistics().fillHistogram(("dLOR_afterEllipCut_"+ EventTypeNames.at(event_type)).c_str(), calculatedLOR( event, event_type));

	      getStatistics().fillHistogram(("sumDiff_azmTheta_afterEllipCut_"+ EventTypeNames.at(event_type)).c_str(), (thetas[0]+ thetas[1]), (thetas[1]- thetas[0]));

	      getStatistics().fillHistogram(("angle_dLOR_afterEllipCut_"+ EventTypeNames.at(event_type)).c_str(), (thetas[0]+ thetas[1]), calculatedLOR( hits, event_type));
	      //getStatistics().fillHistogram(("shortDlor_comp_afterEllipCut_"+ EventTypeNames.at(event_type)).c_str(), calcLOR_sphRadius(event), calcLOR_recsAnnhPt( event, decayPoint ).at(0));

	      getStatistics().fillHistogram(("E1E2_afterEllipCut_"+ EventTypeNames.at(event_type)).c_str(), energies.at(0), energies.at(1));
	      getStatistics().fillHistogram(("hitDis_tDiff_min_afterEllipCut_"+ EventTypeNames.at(event_type)).c_str(), hitDistance_tDiff(hits).first, hitDistance_tDiff(hits).second );
	      getStatistics().fillHistogram(("theta12_afterEllipCut_"+ EventTypeNames.at(event_type)).c_str(), theta_rndm.at(0)*TMath::RadToDeg(), theta_rndm.at(1)*TMath::RadToDeg());
	    */
	      ...
	      //JPetRawMCHit mchit1 = time_window_mc.getMCHit<JPetRawMCHit>( hit1.getMCindex() );
	      //JPetRawMCHit mchit2 = time_window_mc.getMCHit<JPetRawMCHit>( hit2.getMCindex() );
	      //JPetRawMCHit mchit3 = time_window_mc.getMCHit<JPetRawMCHit>( hit3.getMCindex() );

	      //getStatistics().fillHistogram(("mchit_xy_"+ EventTypeNames.at(event_type)).c_str(), mchit1.getPos().X(), mchit1.getPos().Y() );
	      //getStatistics().fillHistogram(("mchit_xz_"+ EventTypeNames.at(event_type)).c_str(), mchit1.getPos().X(), mchit1.getPos().Z() );
	      //getStatistics().fillHistogram(("mchit_z_"+ EventTypeNames.at(event_type)).c_str(), mchit1.getPos().Z() );
	      //getStatistics().fillHistogram(("mchit_x_"+ EventTypeNames.at(event_type)).c_str(), mchit1.getPos().X() );
	      //getStatistics().fillHistogram(("mchit_y_"+ EventTypeNames.at(event_type)).c_str(), mchit1.getPos().Y() );
	      /*
	      getStatistics().fillHistogram(("mchit_t_"+ EventTypeNames.at(event_type)).c_str(), mchit1.getTime() );
       
	      getStatistics().fillHistogram(("res_x_"+ EventTypeNames.at(event_type)).c_str(), hit1.getPos().X() - mchit1.getPos().X() );
	      getStatistics().fillHistogram(("res_y_"+ EventTypeNames.at(event_type)).c_str(), hit1.getPos().Y() - mchit1.getPos().Y() );
	      getStatistics().fillHistogram(("res_z_"+ EventTypeNames.at(event_type)).c_str(), hit1.getPos().Z() - mchit1.getPos().Z() );
	      getStatistics().fillHistogram(("res_t_"+ EventTypeNames.at(event_type)).c_str(), hit1.getTime() - mchit1.getTime() );
	      */
...
/*
TVector3 EventCategorizer::annhPoint(JPetBaseHit* hit1,  JPetBaseHit* hit2, const JPetTimeWindowMC& time_window_mc ){

  JPetMCRecoHit mchit1 = time_window_mc.getMCHit<JPetMCRecoHit>( hit1->getMCindex() );
  JPetMCRecoHit mchit2 = time_window_mc.getMCHit<JPetMCRecoHit>( hit2->getMCindex() );
...
double EventCategorizer::angularRes_rec (  JPetBaseHit* hit1,  JPetBaseHit* hit2, const JPetTimeWindowMC& time_window_mc ){
  

  std::vector<TVector3> momenta;
  momenta.push_back( hit1->getPos() - annhPoint( hit1, hit2, time_window_mc));
  momenta.push_back( hit2->getPos() - annhPoint( hit1, hit2, time_window_mc));

  //momenta.push_back( hit1.getPos() - mchit1.getPolarization());
  //momenta.push_back( hit2.getPos() - mchit2.getPolarization());

  double theta = momenta[0].Angle(momenta[1]);
...
/*
double EventCategorizer::angularRes_gen ( const JPetBaseHit* hit1, const JPetBaseHit* hit2, const JPetTimeWindowMC& time_window_mc ){

  JPetMCRecoHit mchit1 = time_window_mc.getMCHit<JPetMCRecoHit>( hit1.getMCindex() );
  JPetMCRecoHit mchit2 = time_window_mc.getMCHit<JPetMCRecoHit>( hit2.getMCindex() );

  std::vector<TVector3> momenta;
  momenta.push_back( mchit1.getPos() - annhPoint( hit1, hit2, time_window_mc));
  momenta.push_back( mchit2.getPos() - annhPoint( hit1, hit2, time_window_mc));  
...
/*
std::vector<const JPetMCRecoHit*> EventCategorizer::tot( const JPetEvent &event ){
  
  std::vector<const JPetMCRecoHit*> hits_tot;
  
  for(int i=0; i < event.getHits().size(); ++i){
    
    auto singleHit = dynamic_cast<const JPetMCRecoHit*>( event.getHits().at(i) );
    
    getStatistics().fillHistogram("tot_allHits", singleHit->getEnergy() );
    
    if ( singleHit->getEnergy() < 340 ){
...
std::vector<const JPetMCRecoHit*> EventCategorizer::scatTest( std::vector<const JPetMCRecoHit*> hits ) {

  double tDiff;
  double  c = 29.979246;   // cm/ns
  std::vector<double> deltaScatt;
  std::vector<const JPetMCRecoHit*> hits_sTest;
  ...
  for(int i=0; i < hits.size(); ++i){
	  ...
	  if ( fabs(distance - tDiff*c ) < 10){
	    
	    if (hits.at(i)->getTime() < hits.at(j)->getTime()) 
	      hits_sTest.push_back( hits.at(i) );	    
	    else hits_sTest.push_back( hits.at(j) );
...
std::vector<const JPetPhysRecoHit*> EventCategorizer::scatTest( std::vector<const JPetPhysRecoHit*> hits ) {
  ...
  double  c = 29.979246;   // cm/ns
  std::vector<double> deltaScatt;
  std::vector<const JPetPhysRecoHit*> hits_sTest;
...
std::vector<double> EventCategorizer::decayTime_3hits (const JPetEvent *event, EvtType& event_type){
